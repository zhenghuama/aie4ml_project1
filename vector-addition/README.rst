Vector Addition: Streaming vs Buffering
==============================================
----------------------------------------------
Vector Addition with Streaming (vadd-stream)
----------------------------------------------
...
Although the naming convention is not necessarily the most relevant, Vitis needs to know what your main file. In the settings, ``vitis-comp.json``, of the project, appoint ``host.cpp`` as the Top-level File.

Our Top-level File will initialize our graph and control the number of times it is run. But the definition of the dataflow itself is stored in ``graph.hpp``.

The dataflow graph is a class that extends the graph functionality. It can be made up of multiple kernels, snippets of code that will run on the AIE tiles. While multiple kernels can run per tile, a kernel is not split between tiles. 
...
The definition of our ``aie_vadd_stream()`` kernel. It takes in a stream of int32, which is specified by the architecture, places it into a vector of size 4 of types ``int32``, and then uses functions to read continually from the stream and calculate the addition of both streams, storing it into the output stream.

Vitis's analysis is very useful for examining the synthesized data flow. Our final array view shows how the plio blocks stream into the AIE itself and stream the results back into an output. Notice that it does not use any of the tile's local memory.
...
-----------------
Vector Addition with Buffering
-----------------
